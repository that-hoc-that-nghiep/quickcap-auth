...
export const handleUpdateOrg = async (c: Context<{}, '/:orgId', {}>) => {
	...
	const isOwner = user.organizations.some((org: any) => org.id == orgId && org.is_owner);
...
export const handleRemoveUserFromOrg = async (c: Context<{}, any, {}>) => {
	...
	if (user.email === email) {
		throw new BadRequestException('You can not remove yourself, transfer ownership first');
	}
...
export const handleLeaveOrg = async (c: Context<{}, any, {}>) => {
	...
	if (isOwner) {
		throw new BadRequestException('You can not leave organization, transfer ownership first');
	}
