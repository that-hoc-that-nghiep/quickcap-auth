...
export const getUser = async (email: string, db: any) => {
	const user = await db.select().from(users).where(eq(users.email, email)).limit(1).run();
...
export const getUserById = async (id: string, db: any) => {
	const user = await db.select().from(users).where(eq(users.id, id)).limit(1).run();
...
export const getAllUser = async (db: any) => {
	const user = await db.select().from(users).run();

	return user.results;
};

export const updateSubscription = async (id: string, subscription: string, db: any) => {
	const user = await db.update(users).set({ subscription: subscription }).where(eq(users.id, id)).returning().run();
